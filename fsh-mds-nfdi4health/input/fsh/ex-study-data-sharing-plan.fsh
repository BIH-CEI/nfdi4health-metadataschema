...
* extension[generally] ^comment = "Select one value from the list."
* extension[generally].value[x] 1..1
* extension[generally].value[x] only Coding
* extension[generally].value[x].code 1..1
* extension[generally].value[x].system 1..1
* extension[generally].value[x] from $nfdi4health-vs-mds-yes-no-undecided-snomedct-nci (required)
* extension[generally].value[x] ^binding.description = "Value set used to respond to questions that can be answered Yes, No, or Undecided"
...
* extension[supportingInformation] ^comment = "Select all that apply."
* extension[supportingInformation].value[x] 1..1
* extension[supportingInformation].value[x] only Coding
* extension[supportingInformation].value[x].code 1..1
* extension[supportingInformation].value[x].system 1..1 
* extension[supportingInformation].value[x] from $nfdi4health-vs-mds-study-data-sharing-plan-sup-inf-umls-local (required)
* extension[supportingInformation].value[x] ^binding.description = "Value set defining codes to specify if the DataSHIELD/Opal infrastructure is available."
...
* extension[accessCriteria] ^comment = "E.g., with whom, for what types of analyses and by what mechanism the data will be shared."
...
* extension[description] ^comment = "E.g., indication what data in particular will be shared or why the data will not be shared or why it is not yet decided"
...
* extension[datashield].value[x].code 1..1
* extension[datashield].value[x].system 1..1
* extension[datashield].value[x] from $nfdi4health-vs-mds-study-data-sharing-plan-ds-snomedct-umls (required)
* extension[datashield].value[x] ^binding.description = "Value set defining codes to specify if the DataSHIELD/Opal infrastructure is available."
